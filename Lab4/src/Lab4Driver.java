 ...
 */
	 ...
	 */
		//*********************Constants*****************************
		final int[] INCR1 = {1,4,13,40,121,364,1093,3280,9841,29524};
		final int[] INCR2 = {1,5,17,53,149,373,1123,3370,10111,30334};
		final int[] INCR3 = {1,10,30,60,120,360,1080,3240,9720,29160};
		final int[] INCR4 = {1,5,25,50,150,300,900,2700,8100,24300};
		final String DEFAULT_FOLDER = "Lab 4 Input Files";
		final int DEFAULT_TIMES = 10;
		//***********************************************************
		...
		int numTimes=DEFAULT_TIMES;		// Number of times the sort should be run
...
//				for ( int j = 0; j <numTimes ; j ++){
				...
				data = IOMethods.readInfo(filename);
					...
					if (sort == 0){
						data = Heap.Sort(data);
						sortName = "Heap Sort";
						sortTime = Heap.sortTime()/1000;
						h = '!';
					}
					else if (sort == 1) {
						data = shell1.sort(data);
						sortName = "Shell Sort (k1)";
						sortTime = shell1.sortTime()/1000;
						h = '1';
					}
					else if (sort == 2){
						data = shell2.sort(data);
						sortName = "Shell Sort (k2)";
						sortTime = shell2.sortTime()/1000;
						h = '2';
					}
					else if (sort == 3){
						data = shell3.sort(data);
						sortName = "Shell Sort (k3)";
						sortTime = shell3.sortTime()/1000;
						h = '3';
					}
					else if (sort == 4 ){
						data = shell4.sort(data);
						sortName = "Shell Sort (k4)";
						sortTime = shell4.sortTime()/1000;
						h = '4';
					}
					//h = (char)k;
					outFile += h + files[k];
